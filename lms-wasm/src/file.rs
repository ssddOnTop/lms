...
// way easier.
unsafe impl Sync for WasmFileIO {}
unsafe impl Send for WasmFileIO {}
...
impl FileIO for WasmFileIO {
    ...
    async fn create_dirs<'a>(&'a self, _path: &'a str) -> anyhow::Result<()> {
        // Cloudflare Workers KV doesn't have directories
        Ok(())
    }
}
